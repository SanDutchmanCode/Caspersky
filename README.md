...
**Caspersky: The Friendly Ghost** is a nomadic digital entity running on an ESP32 microcontroller. Operating at the intersection of cybersecurity and coffee culture, Caspersky moves through the twilight zone between open visibility and stubborn elusiveness; though the hardware always leaves behind an indelible fingerprint, the ghost makes it nearly impossible for passive observers to pin it down. It is not a malicious tool, but rather a playful artistic statement and a living work of art that dances with the ether—and challenges spectators to scrutinize their digital security.
...
* **MAC-Spoofing:** To reduse passive long-term tracking, the ghost undergoes a forced MAC-layer reincarnation every 5 minutes, randomizing its hardware address while maintaining local unicast standards.
...
**​Hardware Specifications:** While the firmware is compatible with standard ESP32 boards, the project is optimized for and tested on the `Seeed Studio XIAO ESP32S3 (u.FL)`. Utilizing its external u.FL antenna connector allows for extended Wi-Fi reach across long distances (such as spanning from the meter cupboard to the back fence), while the ESP32-S3 architecture natively supports the required eFuse MAC extraction, LittleFS, and mbedTLS AES-GCM operations.
